...
favicon: ## Re-render favicon.ico and apple-touch-icon.png from the SVG sources
	$(MAGICK) -background "#$(KRAFT)" favicon.svg -resize 180x180 \
		-alpha off -colorspace sRGB -type TrueColor -strip apple-touch-icon.png
	$(MAGICK) \
		\( -background "#$(KRAFT)" favicon.svg -resize 48x48 \) \
		\( -background "#$(KRAFT)" favicon.svg -resize 32x32 \) \
		\( -background "#$(KRAFT)" tools/favicon-16.svg -resize 16x16 \) \
